...
class BYOL(nn.Module):
    def __init__(
        ...
        self.augment1 = augment_fn if augment_fn else no_op
        self.augment2 = augment_fn2 if augment_fn2 else no_op
        ...
        self.forward([random_bases(seq_len), random_bases(seq_len)])
    ...
    def forward(
        ...
        assert not (self.training and len(x) == 1), 'you must have greater than 1 sample when training, due to the batchnorm in the projection layer'
        ...
        if negative_examples is not None:
            n1, _ = self.online_encoder(negative_examples)
